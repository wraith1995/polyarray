...
| a degree estimate (whole program) | exists but lives in POINTWISE today: `single_compile/grassmann_backend.py::estimate_degree` → `_program_degree` (statement-graph degree trace: einsum/apply SUM, `+` MAX, vmap `CallOp` recursed; over-estimation safe). **Should become polyarray infrastructure.** KNOWN GAP: specialized to the affine case — non-affine geometry → ∞, and nonlinear/rational ops are only degree-tracked when they touch degree-0 (constant) values |
...
Tests: `pytest -n auto` in this repo's `.venv`. Audit: `python3 ../../audit/run_audits.py
