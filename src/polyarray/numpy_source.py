...
def to_numpy_source(
    ...
    own Stmt ops can be emitted without polyarray depending on them; it takes
    precedence over the op's own ``__numpy_source__`` hook.  An op with no
    built-in renderer, no supplied renderer and no hook raises
    :class:`NotImplementedError` naming the op.
    """
