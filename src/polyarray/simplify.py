...
# ---------------------------------------------------------------------------
# Dead-statement pruning (straight-line, single reverse pass).
# ---------------------------------------------------------------------------

#: Ops whose VALUE may be unused but whose EXECUTION is the point — never pruned.
#: (`AssertOp` raises on a shape/dim mismatch; dropping it would silently delete a check.)
_DCE_EFFECTFUL = ("AssertOp",)


def prune_dead_statements(program, *, recurse: bool = False):
    """Drop statements whose outputs nothing downstream reads — the SIMPLE dead-code pass.

    Deliberately minimal: ONE backward pass over ``program.statements`` in reverse, seeded with the
    program's declared ``outputs``. A statement is LIVE iff some cell it produces is still needed;
    a live statement's inputs then become needed. No fixed point is required because a ``Program``'s
    statements are in topological order (a statement can only read names produced before it), so one
    reverse sweep reaches everything reachable. Names MAY be re-assigned (they are, in practice), so
    the sweep kills a name's need at its definition before generating its operands' needs — standard
    backward liveness. No loop/back-edge handling: this pass is for straight-line bodies only.

    Why this exists: the FEEC certificate's `QrOp`/`_QrSignConventionOp` became VESTIGIAL once the
    sub-entity inclusion was spelled as a direct `Img ⊸ Img` leaf and the `PUSHFORWARD` geometry vars
    were pinned to the coordinate basis they already arrive in — the factorization is still emitted
    but nothing consumes `Q`/`R`. Off the dependency path it cannot block the exact lane, but it
    still costs IR mass and codegen, and it reads to a human as though a radical were in play.

    ``recurse`` also prunes nested sub-``Program`` bodies (a ``Stmt.fn`` that is itself a ``Program``).
    Statements whose op is in :data:`_DCE_EFFECTFUL` are always kept.

    Returns ``(pruned_program, n_removed)``; the program is modified in place and returned, so a
    caller that wants the original must ``copy()`` first.
    """
    def _names(x):
        out = []
        for a in (x if isinstance(x, (list, tuple, set)) else [x]):
            if isinstance(a, str):
                out.append(a)
            else:
    ...
    if not program.outputs:
        # REFUSE. A program whose outputs are held EXTERNALLY (e.g. `symbolic_vandermonde` returns a
        # SymArray that owns the result; `program.outputs` is empty) gives this pass no live seed, and
        # a backward sweep from an empty seed marks EVERYTHING dead. Measured: the FEEC symbolic `W`
        # top-level program went 162 statements -> 0. Refusing is the only safe answer — the pass
        # cannot know what the caller still holds.
        raise ValueError(
            "prune_dead_statements: program.outputs is empty — nothing to seed liveness from, and an "
            "empty seed would prune every statement. Pass a program whose outputs are registered, or "
            "prune the sub-programs individually.")

    needed: set[str] = set()
    for sa in program.outputs.values():
        needed.update(_names(getattr(sa, "name", None) or []))
    needed.update(program.outputs.keys())

    live: set[int] = set()
    for i in range(len(program.statements) - 1, -1, -1):
        s = program.statements[i]
        outs = _names(s.out)
        keep = type(s.fn).__name__ in _DCE_EFFECTFUL or any(o in needed for o in outs)
        # KILL before GEN: a statement DEFINES its outs, so walking backward it satisfies every
        # need for them seen so far. Names ARE re-assigned in real bodies (measured: `f_cod_dom`
        # produced at two statements of one `grass_dof`), so without the kill an earlier producer
        # of the same name would be kept alive by a need its later redefinition already answered.
        needed.difference_update(outs)
        if keep:
            live.add(i)
            needed.update(_names(s.in_))
    removed = len(program.statements) - len(live)
    if removed:
        program.statements = type(program.statements)(
            s for i, s in enumerate(program.statements) if i in live)
    if recurse:
        for s in program.statements:
            sub = s.fn if hasattr(s.fn, "statements") else getattr(s.fn, "fn", None)
            if sub is not None and hasattr(sub, "statements"):
                _, r = prune_dead_statements(sub, recurse=True)
                removed += r
